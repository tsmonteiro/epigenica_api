champ.filter.epi <- function(beta=myImport$beta,
                         M=NULL,
                         pd=myImport$pd,
                         intensity=NULL,
                         Meth=NULL,
                         ...
                         arraytype = "450K")
{
  ...
  if(FilterOption$filterSNPs== TRUE)
  {
    ...
    }else
    {
      if(is.null(population))
      {
        ...
        load('./EPIC_manifest.Rda')
        # annoGFile <- drive_get('Epigenica/Dados/EPIC_manifest.Rda')
        # anno_tmp <- tempfile()
      ...
      }else
      {
        ...
        load('./EPIC_manifest_pop.Rda')
        # annoGFile <- drive_get('Epigenica/Dados/EPIC_manifest_pop.Rda')
        # anno_tmp <- tempfile()
        # dfile <- drive_download(annoGFile$id, path=anno_tmp)
        # load(dfile$local_path)
        # unlink(anno_tmp)
  ...
  if(FilterOption$filterMultiHit == TRUE)
  {
    ...
    # data(multi.hit)
    load('./multi.hit.Rda')
    # annoGFile <- drive_get('Epigenica/Dados/multi.hit.Rda')
    # anno_tmp <- tempfile()
    # dfile <- drive_download(annoGFile$id, path=anno_tmp)
    # load(dfile$local_path)
    # unlink(anno_tmp)
